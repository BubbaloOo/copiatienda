...
async function registrarClick() {
  const COUNT_KEY = 'countapi_hashirafigure_alex_v1'; // clave en localStorage
  const EP = 'https://api.countapi.store/v2/hashirafigure/alex/increment';

  console.log('registrarClickOnce: start');

  try {
    // Verificar si ya se ha registrado un click
    if (localStorage.getItem(COUNT_KEY)) {
      console.log('registrarClickOnce: ya se registró un click anteriormente. No se envía.');
      return null;
    }

    // Enviar POST a CountAPI
    console.log('registrarClickOnce: enviando POST a', EP);
    const r = await fetch(EP, { method: 'POST' });
    console.log('registrarClickOnce: response ok?', r.ok, r.status);

    if (!r.ok) {
      const bodyError = await r.text().catch(() => '<no body>');
      console.error('registrarClickOnce: respuesta no OK', r.status, bodyError);
      return null;
    }

    // Marcar localStorage solo si la petición fue exitosa
    try {
      localStorage.setItem(COUNT_KEY, '1');
      console.log('registrarClickOnce: click registrado en localStorage correctamente');
    } catch (e) {
      console.warn('registrarClickOnce: no se pudo escribir en localStorage', e);
    }

    const body = await r.text().catch(() => null);
    console.log('registrarClickOnce: body:', body);
    return body;

  } catch (err) {
    console.error('registrarClickOnce: fetch error:', err);
    // No marcamos localStorage para permitir reintento
